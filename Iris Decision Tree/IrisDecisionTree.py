...
def importData():
    ...
    iris_data: Data from the csv file
    '''
    iris_data = pd.read_csv(r'iris.data', sep= ',', header = None)
...
def dataSlicing(iris_data):
    '''Function to split the dataset into Training and Test Sets
    
    Params
    ------
    iris_data: DataFrame containing the entire dataset
    ...
    '''
    X = iris_data.values[:, 0:4]        #Attributes from dataset
    Y = iris_data.values[:, 4]           #Target Values for respective Atributes
